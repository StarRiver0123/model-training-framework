...
from src.modules.models.text_c import Bert_C, Lstm_C, SelfDefinedBert_C
...
class TrainingPureStudentModel(nn.Module):
    def __init__(self, config, word_vector=None):
        ...
        if (used_model == 'self_bert_c'):
            resource_root = config['resource_root']
            bert_config_file = resource_root + os.path.sep + config['net_structure']['student_model_config_file']
            self.model = SelfDefinedBert_C(bert_config_file, num_classes=num_classes).to(device)
        if (used_model == 'lstm_c'):
            ...
            if word_vector is not None:
                self.model.embedding.from_pretrained(embeddings=word_vector,
                                                     freeze=not pretrained_word_vector_full_fine_tuning,
                                                     padding_idx=pad_idx)
...
class TrainingDistilledStudentModel(nn.Module):
    def __init__(self, config, word_vector=None):
        ...
        if (used_teacher_model == 'bert_c'):
            ...
            self.teacher_model.bert_model.config.output_hidden_states = True
        if (used_student_model == 'self_bert_c'):
            resource_root = config['resource_root']
            bert_config_file = resource_root + os.path.sep + config['net_structure']['student_model_config_file']
            self.model = SelfDefinedBert_C(bert_config_file, num_classes=num_classes).to(device)
            self.model.bert_model.config.output_hidden_states = True
        ...
        if used_lr_scheduler == 'cosdecay':
            ...
            # self.lr_scheduler = CosDecayLRScheduler(self.optimizer, step_size=lr_step_size, epochs=epochs, num_examples=train_set_size, batch_size=batch_size, min_lr=min_lr, warmup_size=warmup_size)
            train_steps_per_epoch = train_set_size // batch_size
            self.lr_scheduler_class = get_cosine_schedule_with_warmup
            self.lr_scheduler_args = {'num_warmup_steps': train_steps_per_epoch, 'num_training_steps': epochs * train_steps_per_epoch}
        else:
            self.lr_scheduler = None
...
def create_distilled_student_inference_model(config):
    ...
    if (used_model == 'self_bert_c'):
        resource_root = config['resource_root']
        bert_config_file = resource_root + os.path.sep + config['net_structure']['student_model_config_file']
        model = SelfDefinedBert_C(bert_config_file, num_classes=num_classes).to(device)
    model.eval()
    return model
