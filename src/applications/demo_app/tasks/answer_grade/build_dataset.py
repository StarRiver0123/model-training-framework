...
def load_train_valid_split_set(arguments):
    ...
    train_q_file = arguments['net_structure']['dataset']['train_q_file']
    train_a_file = arguments['net_structure']['dataset']['train_a_file']
...
def load_test_set(arguments):
    # return test set
    dataset_root = arguments['dataset_root']
    test_q_file = arguments['net_structure']['dataset']['test_q_file']
    test_a_file = arguments['net_structure']['dataset']['test_a_file']
...
def get_data_iterator(arguments, train_set=None, valid_set=None, test_set=None):
    ...
    # compute the field
    if not use_bert:
        fun_name = arguments['net_structure']['tokenizer']
        ...
        arguments['dataset']['symbol'].update({'sos_token': tokenizer.cls_token,
    ...
    if train_set is not None:
        ...
        if not use_bert:
            vector_file = word_vector_root + os.path.sep + arguments['net_structure']['word_vector_file']
            word_vectors = Vectors(name=vector_file)
            TEXT_FIELD.build_vocab(train_examples, vectors=word_vectors)
            arguments['dataset']['symbol'].update({'unk_idx': TEXT_FIELD.vocab.stoi[unk_token],
        ...
        if valid_set is not None:
            valid_examples = getDataExamples_withTorchText(valid_set, TEXT_FIELD, TEXT_FIELD)
    ...
    if test_set is not None:
        test_examples = getDataExamples_withTorchText(test_set, TEXT_FIELD, TEXT_FIELD)
